...
class Mapping extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->load->model('_Mapping');
    }
    public function index()
    {
        $data['access_map'] = $this->_Mapping->_view_all_access_map();
        $data['parent_map'] = $this->_Mapping->_view_all_parent_map();
        $data['count_access_map'] = $this->_Mapping->_count_access_map();
    ...
    public function create()
    {
        if (isset($_POST['new_parent_map'])) {
            ...
            $check = $this->_Mapping->_check_parent_map($data);
            if ($check['status'] == 0) {
                $dataRs = $check;
            } else {
                $dataRs = $this->_Mapping->_create_parent_map($data);
            }
            ...
            $check = $this->_Mapping->_check_access_map($data);
            if ($check['status'] == 0) {
                $dataRs = $check;
            } else {
                $dataRs = $this->_Mapping->_create_access_map($data);
            }
        }
        $dataRs['access_map'] = $this->_Mapping->_view_all_access_map();
        $dataRs['parent_map'] = $this->_Mapping->_view_all_parent_map();
        $dataRs['count_access_map'] = $this->_Mapping->_count_access_map();
        $this->load->view('Mapping/Index', $dataRs);
    }
    public function read()
    {
        // Here You Code for Read
        $id = $_GET['id'];
        $dataRs['access_map'] = $this->_Mapping->_view_access_map($id);

        $this->load->view('Mapping/Read', $dataRs);
        $this->load->view('Templates/Footer');
    }
    public function update()
    {
        ...
        if (isset($_POST['btn_parent_map'])) {
            ...
            $dataRs = $this->_Mapping->_update_parent_map($id, $data);
        } else if (isset($_POST['btn_access_map'])) {
            $data = array(
                'access_map' => str_replace(' ', '_', $_POST['access_map']),
                'parent_map_id' => $_POST['parent_map_id']
            );
            $dataRs = $this->_Mapping->_update_access_map($id, $data);
        }
        $data = $dataRs;
        $data['access_map'] = $this->_Mapping->_view_all_access_map();
        $data['parent_map'] = $this->_Mapping->_view_all_parent_map();
