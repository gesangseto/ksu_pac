...
class User_System extends CI_Controller
{
    public function __construct()
    {
        ...
        $this->load->model('_User_System');
    }


    public function index()
    {
        $data['user'] = $this->_User_System->_view_all_user();
        $this->load->view('User_System/Index', $data);
    ...
    public function create()
    {
        ...
        if (empty($this->input->post('email'))) {
            $this->load->view('User_System/Create');
        } else {
            $email = $this->input->post('email', TRUE);
            $username = $this->input->post('username', TRUE);
            $query = $this->db->query("SELECT * FROM `system_admin` WHERE `email` = '$email' OR `username`='$username'");
            if ($query->num_rows() >= 1) {
                $data = array(
                    "status" => "0",
                    "message" => "Email or username already exist"
                );
                $this->load->view('User_System/Create', $data);
            } // Check Password 
    ...
    public function read()
    {
        if (!empty($this->input->get('id'))) {
            ...
            $data['user_info'] = $this->_User_System->_view_user($id);
            $this->load->view('User_System/Read', $data);
        }
        $this->load->view('Templates/Footer');
    }
    public function update()
    {
        if (!empty($this->input->get('id'))) {
            $id = $this->input->get('id', TRUE);
            $data['user_info'] = $this->_User_System->_view_user($id);
            $this->load->view('User_System/Update', $data);
            $this->load->view('Templates/Footer');
            ...
            if (!empty($this->input->post('password') && $this->input->post('re_password'))) {
                if ($this->input->post('password') != $this->input->post('re_password')) {
                    ...
                    $this->load->view('User_System/Update', $dataRs);
                } else {
                    $data = array(
                        'id' => $this->input->post('id'),
                        'username' => $this->input->post('username'),
                    ...
                    $check = $this->_User_System->_check_update($id, $data);
                    if ($check['status'] == 1) {
                        $data = $this->_User_System->_update_user($id, $data);
                        $data['user'] = $this->_User_System->_view_all_user();
                        $this->load->view('User_System/Index', $data);
                    } else {
                        $dataRs = $check;
    ...
    {
        $id = $this->input->get('id', TRUE);
        $this->db->delete('user_role', array('admin_id' => $id));
        $this->db->delete('system_admin', array('id' => $id));
