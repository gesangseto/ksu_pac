...
class _Permission extends CI_Model
{
    public function _view_all_role()
    {
        ...
        FROM `system_admin` AS `A` JOIN `user_role` AS `B` ON `A`.`id` = `B`.`admin_id` 
        GROUP BY `A`.`username` ASC");
        return ($query->result_array());
    }
    public function _view_all_user()
    {
        $query = $this->db->query("SELECT * FROM `system_admin`  GROUP BY `username` ASC");
        return ($query->result_array());
    }
    ...
    public function _view_user_role($id)
    {
    ...
    {
        $this->db->where('id', $id);
        $query = $this->db->update('user_role', $field);
        if ($query == 1) {
            $data = array(
                'status' => '1',
    ...
    {
        $query = $this->db->get_where('user_role', array('admin_id' => $field['admin_id'], 'access_map_id' => $field['access_map_id']));
        if ($query->num_rows() == 0) {
            $data = array(
                'status' => '1',
                'message' => 'success update data'
    ...
    {
        $query = $this->db->insert('user_role', $field);
        if ($query) {
            $data = array(
                'status' => '1',
